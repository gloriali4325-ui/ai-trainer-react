...
async function loadOperationalQuestions(
  ...
  const now = nowIso();
  const response = await fetch('/ai-trainer-react/assets/operational_skills.json');
...
async function loadFromAssets(): Promise<{
  ...
  const now = nowIso();
  const response = await fetch('/ai-trainer-react/assets/ai_trainer_bank_v2.json');
  const decoded = (await response.json()) as Array<Record<string, unknown>>;

  const operationalResponse = await fetch('/ai-trainer-react/assets/operational_skills.json');
