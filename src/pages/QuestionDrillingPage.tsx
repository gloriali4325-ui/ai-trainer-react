...
export function QuestionDrillingPage() {
  const { theoryQuestions, profile, recordAttempt } = useApp();
  ...
  const storageKey = profile ? `drilling_progress_${profile.id}` : '';
  const storageStateKey = profile ? `drilling_progress_state_${profile.id}` : '';

  const questions = useMemo<Question[]>(
    () =>
      questionOrder.length
        ? questionOrder
            .map((id) => theoryQuestions.find((q) => q.id === id))
            .filter(Boolean) as TheoryQuestion[]
        : shuffle(theoryQuestions),
    [questionOrder, theoryQuestions],
  );

  useEffect(() => {
    if (!storageKey || !storageStateKey || theoryQuestions.length === 0) return;
    const savedState = readJson<DrillingState | null>(storageStateKey, null);
    if (savedState) {
    ...
    const shuffled = shuffle(theoryQuestions).map((q) => q.id);
    setQuestionOrder(shuffled);
    const storedIndex = readJson<number>(storageKey, 0);
    setIndex(storedIndex);
  }, [storageKey, storageStateKey, theoryQuestions]);
      ...
      <div className="split-layout">
        <div>
          <div className="panel" style={{ marginBottom: 12, display: 'flex', justifyContent: 'space-between', gap: 12 }}>
            <div>
              <strong>随机练习 · 理论</strong>
              <div className="muted">单选 / 多选 / 判断</div>
            </div>
